...
Vision Len is a mobile-first AI visual assistant for visually impaired users that describes scenes, flags hazards, answers questions about surroundings, and gives spoken walking directions — all hands-free with voice wake word and multilingual output.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "230826738+karpohkpt-collab@users.noreply.github.com"
  git config user.name "karpohkpt-collab"
  ```
